# Data-Insight-dashboard (Optimism Mainnet Analytics)

<h1 align="center">
   <b>
        <a href="https://data-insight-dashboard.vercel.app/"><img src="https://github.com/holyaustin/DataInsight-Dashboard/blob/main/public/datalogo.png" /></a><br>
    </b>
</h1>

<p align="center">
    <a href="https://data-insight-dashboard.vercel.app/"><b>Website</b></a> •
    <a href="https://github.com/holyaustin/DataInsight-Dashboard/blob/main/README.md"><b>Documentation</b></a>
</p>

# Data-Insight-dashboard (Optimism Mainnet Analytics)

Providing a visual analytics front-end with rich information about the health and performance of Dex on Optimism ecosystem - Powered by Optimism, Covalent and Coingecko APIs.
...
- For the success of a DEX protocol, running on an XY=K engine, a visual analytics front-end dashboard is needed to provide users with rich information about the health and performance of that ecosystem. Optimism is a fast growing ecosystem with a decentralized exchange built in the ecosystem, thus we approach to solve the problem by providing  [EddAlytics](https://data-insight-dashboard.vercel.app/) - Optimism Dex Dashboard Analytics.
...
- [ ] Add more DEX protocol on Optimism mainnet and expand to other chains
- [ ] Add wallet portfolio page to display users asset
...
```bash
git clone https://github.com/holyaustin/DataInsight-Dashboard
```
...
- note: `const chainStatus = data?.data?.items[25].synced_block_height` at `[25]` we the get the chain id of Optimism in array of objects 😊
...
- notes: we simply provide the name of the exchange `/cronus/` and the correct chainID (9001 = Optimism_mainnet) `/9001/` for the blockchain. Lastly your Api key from [Covalent](https://www.covalenthq.com/) you provided on .env.local as a variables `/?&key=${APIKey}` , as a result, these endpoints enable you to access any DEX-related protocol in a matter of minutes, cool hey 😊

### Get Top pools by price volume liquidity

API method `/9001/xy=k/Optimismwap/pools` can be used to get information on tokens by price, volume, liquidity,

- notes: we simply provide the name of the exchange `/Optimismwap/` and the correct chainID (9001 = Optimism_mainnet) `/9001/` for the blockchain. Lastly your Api key from [Covalent](https://www.covalenthq.com/) you provided on .env.local as a variables `/?&key=${APIKey}` ,as a result, these endpoints enable you to access any DEX-related protocol in a matter of minutes, Super great!😊
...
export default function UniswapPools() {
  ...
  const { data, error, isFetching } = useQuery(["UniswapPools"], async () => {
    const res = await fetch(
      `https://api.covalenthq.com/v1/9001/xy=k/Uniswap/pools/?key=${APIKey}`
    );
    return res.json();
  });
...
API XY=K method `/v1/9001/xy=k/Uniswap/tokens/address/0x181c262b973b22c307c646a67f64b76410d19b6b/transactions/` can be used to get transactions information on account address, which also includes these type of transactions eg. SWAP, ADD LIQUIDITY, REMOVE LIQUIDITY.

- notes: we simply provide the name of the exchange `/Uniswap/` and the correct chainID (10 = Optimism_mainnet) `/9001/` for the blockchain, and you will note that here we have account, an address, which represents Uniswap address - which will help us get all the transactions on this address. Lastly your Api key from [Covalent](https://www.covalenthq.com/) you provided on .env.local as a variables `/?&key=${APIKey}` ,as a result, these endpoints enable you to access any DEX-related protocol in a matter of minutes, Nice!😊
...
export default function UniswapTransactions() {
  ...
  const { data, error, isFetching } = useQuery(
    ...
    async () => {
      const res = await fetch(
        `https://api.covalenthq.com/v1/9001/xy=k/Uniswap/tokens/address/0x181c262b973b22c307c646a67f64b76410d19b6b/transactions/?key=${APIKey}`
      );
      return res.json();
    }
  );
...
```
#### Demo Website:<https://data-insight-dashboard.vercel.app/>

#### GitHub Repository: <https://github.com/holyaustin/DataInsight-Dashboard>

#### Demo video: [video](https://youtu.be/-U7J8TYqHao)
